...
Tangling this document will emit 2 elisp libraries; the files
~dot-emacs-base.el~ and ~dot-emacs.clojure.el~. Evaluating the
following code block (use C-c C-c when the cursor is in the source
block), which itself is not tangled, will after tangling also
byte-compile the libraries.
...
In the emacs init file (like ~/.emacs), create a section with the
following content to load the desired elisp libraries. The
~dot-emacs-base~ library is required and must be loaded first. The
~dot-emacs-clojure~ library is only useful when doing clojure or
clojurescript development and can be loaded after the base
configuration file (it is depending on functionality defined in the
~dot-emacs-base~ library, which must therefor be loaded first).

Be sure to change to PATH-TO-THIS-REPOSITORY to point to the root
