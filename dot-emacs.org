...
The following enables basically the same for ~shell~ by introducing a
new function.

#+begin_src elisp
  (defun qrt/shell-at-current-dired-dir
      ()
    (interactive)
    (let ((default-directory (if (eq major-mode 'dired-mode)
      ...
      (funcall-interactively 'shell (concat "shell - " default-directory))))

  (global-set-key (kbd "C-æ d s") 'qrt/shell-at-current-dired-dir)
#+end_src
...
task to work on, and notfies when it expires. Asks for a task title
and an amount of minutes to work on the task (defaults to 25 minutes).
...
The following function starts a pomodoro for a selected org-heading.
The task name is derived from the current org-heading. The amount of
minutes to use on the task is proposed to be 25 minutes. The org
buffer is automatically narrowed to the subtree for the current
org-header, whith only its direct children being shown.

#+begin_src elisp
  (defun qrt/org-heading->pomodoro
      ()
    (interactive)
    (let* ((task (substring-no-properties (org-get-heading t t t t)))
           (time (read-number (concat "minutes to work on '" task "': ") 25)))
      (org-narrow-to-subtree)
      (org-fold-hide-subtree)
      (org-fold-show-entry)
      (org-fold-show-children)
      (qrt/start-pomodoro task time)))
#+end_src
