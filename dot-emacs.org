...
As we use ~load-library~, and not ~load-file~, we can also take
advantage of emacs native compilation on our own elisp. To do so, just
execute the following source block in stead of the one used above for
just tangling this document.

#+begin_src elisp :tangle no :results silent
  (org-babel-tangle)
  (native-compile "./dot-emacs-base.el")
  (native-compile "./dot-emacs-clojure.el")
#+end_src
