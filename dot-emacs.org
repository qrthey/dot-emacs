...
the following example to load the desired elisp libraries. The
...
clojurescript development (it is depending on functionality defined in
the ~dot-emacs-base~ library, which must therefor be loaded first).

Be sure to change to PATH-TO-THIS-REPOSITORY in the example to point
