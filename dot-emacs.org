...
functionality. As a bonus, emacs should start up rather fastly with
this configuration.
...
In a same manner, custom keybindings typicaly start with ~C-æ~ as
this special danish character is unlikely to be used by any package.
...
example pre-lude above) as this allows usage of different themes on
different machines, without the need to first onload a previously
loaded theme.
